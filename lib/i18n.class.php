...
class i18n {
    ...
    static public function getConstantIntl($constant_name)
    {
        if (_DEV_MODE_ && !defined($constant_name)) {
            //var_dump($constant_name.' missing');
        }
        return defined($constant_name) ? constant($constant_name) : $constant_name;
    }

    static public function translateArray(&$array, $parents_key = '') {
        foreach($array as $key => &$data) {
            if (is_array($data)) {
                self::translateArray($data, $parents_key.'_'.$key);
            } else {
                $data = self::getConstantIntl('L::'.$parents_key.'_'.$key);
            }
        }
    }
}
